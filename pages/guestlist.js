...
const Guestlist = () => {
    ...
    return (
      <div
        className={`flex-1 p-4 border rounded-xl ${
          guest.rsvp === "notAttending" ? "border-red-500" : "border-green-500"
        }`}
      >
        <p className={"font-semibold"}>Guests</p>
        <p key={guest.key}>{guest.name}</p>
        <p className={"font-semibold"}>RSVP Status</p>
        <p className="capitalize" key={guest.key}>
          {guest.rsvp}
        </p>
        {guest?.dietReqs && (
          <>
  ...
  const numberNotComing = 0;
  // list && list?.filter((i) => i.rsvp === "notAttending").length;
    ...
    <div className={"container mx-auto"}>
      ...
      <div>
        <p className={"text-xl font-bold"}>
          Number of Guests Coming: {list && list.length - numberNotComing}
        </p>
        <p className={"text-xl font-bold"}>
          Number of Guests Not Coming: {numberNotComing}
        </p>
        <p className={"text-xl font-bold"}>
          Total Number of Guests: {list && list.length}
        </p>
