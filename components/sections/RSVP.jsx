...
const Rsvp = () => {
    ...
    const rsvpOptions = [
        { id: 'attending', value:"attending", name: 'Attending', description: "We'll be there & have NO dietary requirements" },
        { id: 'attendingWithReqs', value:"attendingWithReq", name: 'Attending w/ Dietary Requirements', description: "We'll be there & have some dietary requirements" },
        { id: 'notAttending', value:"notAttending", name: 'Not Attending ', description: 'Sorry, we can not make it :( ' },
    ]
    ...
    return (
        <div className='container mx-auto'>
                ...
                <form onSubmit={handleSubmit}>
                        ...
                        <fieldset className="mt-2">
                            ...
                            <div className="divide-y divide-gray-200">
                                {rsvpOptions.map((option, optionIdx) => (
                                    <div key={optionIdx} className={`relative flex justify-center items-start p-4 ${option.id === rsvp ? "bg-slate-50" : ""} `}>
                    ...
                    {isSent && (
                        <div className={'flex flex-row items-center justify-center bg-green-100 rounded-xl p-4 my-3 text-slate-700'}>
                            <HandThumbUpIcon className="h-8 w-6 mr-4"/>
                            <p>RSVP Successfully Sent | <Link passHref href={"/accommodation"}><a><span className="text-slate-500 text-sm">View Accommodation </span></a></Link> </p>
                        </div>
                    )}
                    {isError && (
                        <div className={'flex flex-row items-center justify-center bg-red-100 rounded-xl p-4 my-3 text-slate-700'}>
                            <HandThumbDownIcon className="h-8 w-6 mr-4"/>There was an error :(
                        </div>
                    )}

                    <Button className={`my-4 ${isSending && 'bg-orange-600'} ${isSent && "bg-green-600"} ${isError && "bg-red-600"}`} type="submit">
