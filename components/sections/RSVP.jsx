    ...
    <div className="container mx-auto">
      ...
      <form onSubmit={handleSubmit}>
          ...
          <fieldset className="mt-2">
            ...
            <div className="divide-y divide-gray-200">
              {rsvpOptions.map((option, optionIdx) => (
                <div
                  key={optionIdx}
                  className={`relative flex justify-center items-start p-4 ${
                    option.id === rsvp ? "bg-slate-50" : ""
                  } `}
                >
        ...
        {isSent && (
          <div
            className={
              "flex flex-row items-center justify-center bg-green-100 rounded-xl p-4 my-3 text-slate-700"
            }
          >
        ...
        {isError && (
          <div
            className={
              "flex flex-row items-center justify-center bg-red-100 rounded-xl p-4 my-3 text-slate-700"
            }
          >
            <HandThumbDownIcon className="h-8 w-6 mr-4" />
            There was an error :(
          </div>
        )}

        <Button
          className={`my-4 ${isSending && "bg-orange-600"} ${
            isSent && "bg-green-600"
          } ${isError && "bg-red-600"} ${
            hasNoData || isSending || (isSent && "cursor-not-allowed")
          }`}
          type="submit"
        ...
        >
          ...
          {!isSent && !isSending && !isError && (
            <EnvelopeOpenIcon className="h-6 w-6 text-white mr-2 " />
          )}
          <p>RSVP</p>
        </Button>
      </form>
